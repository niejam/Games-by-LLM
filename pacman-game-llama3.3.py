...
pygame.display.set_caption("Police and Innocent Man with Power Up feature")
...
POWER_ICON = pygame.image.load("assets/power.png")
...
def main():
    ...
    power_icon_position = (8, 9)  # Position of the power icon
    ...
    while running:
        ...
        if grid[pacman.y][pacman.x] == 2:  # Reached exit
            print("You Win!")
            running = False
        if (pacman.x, pacman.y) == power_icon_position:  # Pick up power icon
            lives += 1
            power_icon_position = None  # Remove power icon
        ...
        for ghost in ghosts:
            ghost.draw()
        if power_icon_position:
            screen.blit(pygame.transform.scale(POWER_ICON, (TILE_SIZE, TILE_SIZE)), (power_icon_position[0] * TILE_SIZE, power_icon_position[1] * TILE_SIZE))
        draw_ui(score, lives)
